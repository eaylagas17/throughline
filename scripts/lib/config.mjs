...
// the default rather than throwing, since the surface hook must never crash a session.
...
export function writeSurfaceMode(storeDir, mode) {
  if (!SURFACE_MODES.includes(mode)) {
    throw new Error(`invalid surface mode "${mode}" (expected one of: ${SURFACE_MODES.join(', ')})`);
  }
